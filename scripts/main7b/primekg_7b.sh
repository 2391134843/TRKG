...
TS=$(date +%F-%H%M.%S)
S1_OUT="${CKPT_ROOT}/PrimeKG_${JOB}_stage1_${TS}"
S2_GRPO_OUT="${CKPT_ROOT}/PrimeKG_${JOB}_grpo_${TS}"
CAND_DIR="${CAND_ROOT}/PrimeKG_${JOB}"
...
log_banner "Stage1 7B | PrimeKG | param-C | GPU=${DS_GPU}" \
...
log_banner "Stage2 7B | PrimeKG | param-C | candidate-gen | GPU=${DS_GPU}" | tee -a "${JOB_LOG}" "${CAND_LOG}"
...
log_banner "Stage2 7B | PrimeKG | param-C | GRPO | GPU=${DS_GPU}" | tee -a "${JOB_LOG}" "${GRPO_LOG}"
