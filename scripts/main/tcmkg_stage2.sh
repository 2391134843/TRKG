...
GRPO_DIR="${CKPT_ROOT}/TCMKG_main_grpo_$(date +%F-%H%M.%S)"
mkdir -p "${CAND_DIR}" "${GRPO_DIR}"

log_banner "Main Stage 2 | TCMKG | Qwen2.5-0.5B(-Instruct) | ckpt=${BEST_CKT}" | tee -a "${LOG_FILE}"
